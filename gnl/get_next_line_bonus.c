...
/*   Updated: 2021/11/26 13:07:25 by avoltorb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_next_line(char *cache)
{
	...
	if (cache[nidx])
	{
		...
		if (!next_line)
			return (NULL);
		ft_strcpy(next_line, cache, nidx);
		next_line[nidx] = '\0';
	}
	return (next_line);
}
...
char	*ft_trim_line(char *cache)
{
	...
	if (cache[nidx])
	{
		...
		if (!new_cache)
			return (NULL);
		new_cache[new_len] = '\0';
		ft_strcpy(new_cache, &cache[nidx + 1], new_len);
		free(cache);
	}
	return (new_cache);
}
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 0)
		return (NULL);
	cache[fd] = ft_read(cache[fd], fd);
	if (cache[fd])
	{
		next_line = ft_next_line(cache[fd]);
		cache[fd] = ft_trim_line(cache[fd]);
		return (next_line);
	}
	return (NULL);
}
